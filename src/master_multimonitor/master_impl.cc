...
bool MasterImpl::Recover() {
    ...
    while (it->Valid()) {
        ...
        for (int i = 0; i < cell.m_conf_size(); i++) {
            MonitorConfig conf;
            conf.monitor_name = cell.m_conf(i).monitor_job();
            conf.monitor_conf = cell.m_conf(i).monitor_conf();
            job_info.monitor_confs.push_back(conf);
        }
...
void MasterImpl::NewJob(::google::protobuf::RpcController* /*controller*/,
    ...
    for (int i = 0; i < request->m_conf_size(); i++) {
        MonitorConfig conf;
        conf.monitor_name = request->m_conf(i).monitor_job();
        conf.monitor_conf = request->m_conf(i).monitor_conf();
        job.monitor_confs.push_back(conf);
    }
...
bool MasterImpl::ScheduleTask(JobInfo* job, const std::string& agent_addr) {
    ...
    if (!ret || (rt_response.has_status() 
        ...
        for (uint32_t i = 0; i < job->monitor_confs.size(); i++) {
            MonitorConf* conf_ptr = instance.add_m_conf();
            conf_ptr->set_monitor_job(job->monitor_confs[i].monitor_name);
            conf_ptr->set_monitor_conf(job->monitor_confs[i].monitor_conf);
        }
...
bool MasterImpl::PersistenceJobInfo(const JobInfo& job_info) {
    ...
    for (uint32_t i = 0; i < job_info.monitor_confs.size(); i++) {
        MonitorConf* monitor_conf_ptr =cell.add_m_conf();
        monitor_conf_ptr->set_monitor_job(job_info.monitor_confs[i].monitor_name);
        monitor_conf_ptr->set_monitor_conf(job_info.monitor_confs[i].monitor_conf);
    }
